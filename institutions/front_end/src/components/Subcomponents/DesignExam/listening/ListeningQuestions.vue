      ...
      <v-container fluid style="padding: 0">
        <v-row>
          ...
          <v-col cols="12" sm="4" md="6" lg="6" xl="6" style="text-align: right; padding: 0">
            <v-btn icon
                   width="60" height="60" to="/add_listening_question">
        ...
        <v-container fluid>
          <v-row>
            <v-col cols="12" sm="8" md="6" lg="6" xl="6">
              ...
              <h4 style="color: rgba(0, 0, 0, 0.5)">Question Audio</h4>
              <audio id="question" preload="metadata" controls>
                  <source :src="question['listening_question_audio_file']"> <!-- audio file-->
                </audio>
              <h4 style="color: rgba(0, 0, 0, 0.5)"  v-if="question.quote">Quote Audio</h4>
               <audio id="quote" preload="metadata" controls v-if="question.quote">
                  <source :src="question['quote_audio_file']"> <!-- audio file-->
                </audio>
...
import {GET_LISTENING_QUESTIONS, DELETE_LISTENING_QUESTION} from "@/store/actions/listening";
...
export default {
  ...
  computed: {
    ...mapGetters(['listeningQuestionsArray']),
    rawList() {
      return this.listeningQuestionsArray.filter(question => {
  ...
  created() {
    ...
    this.handleResize();
    this.$store.dispatch(GET_LISTENING_QUESTIONS).then(() => {
      this.loading = false
    })
  ...
  methods: {
    ...
    deleteQuestion() {
      ...
      this.loading = true
      this.$store.dispatch(DELETE_LISTENING_QUESTION, this.deleteId).then(() => {
        this.$store.dispatch(GET_LISTENING_QUESTIONS).then(() => {
          this.loading = false
        })
        this.snackbarColor = "success"
